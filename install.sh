cat > /tmp/opudp_install.sh <<'SCRIPT'
#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║     OFFICIAL ONEPESEWA UDP Installer                        ║
# ║     Debian/Ubuntu  –  ZIVPN v1.4.9  +  Panel v2.6.0        ║
# ║     @OfficialOnePesewa                                      ║
# ╚══════════════════════════════════════════════════════════════╝
...
DEBIAN_FRONTEND=noninteractive apt-get install -y -qq -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" \
    jq iptables-persistent netfilter-persistent openssl vnstat bc python3-pip conntrack wget curl 2>/dev/null
ok "jq bc openssl vnstat iptables-persistent python3-pip conntrack installed"

# Step 2 – Architecture
step 2 7 "Detecting system architecture"
ARCH=$(uname -m)
case $ARCH in
...
step 3 7 "Downloading ZIVPN v1.4.9"
...
if wget -q --show-progress -O /usr/local/bin/zivpn "$ZIVPN_URL" 2>/dev/null || \
   wget -qO /usr/local/bin/zivpn "$ZIVPN_URL"; then
...
step 4 7 "Setting up config, SSL & kernel"

mkdir -p /etc/zivpn
touch /etc/zivpn/users.db /etc/zivpn/usage.db
touch /etc/zivpn/paused.db /etc/zivpn/conn_limits.db /etc/zivpn/ip_limits.db
...
# Step 7 – Download OPUDP Panel (v2.6.0)
step 6 7 "Installing OPUDP Panel"
...
echo ""
echo -e "  ${YLW}${BOLD}▸ Enable background services?${RST}  ${DIM}(pause / IP limit / bandwidth monitoring)${RST}"
echo -ne "  ${CYN}  [y/N]: ${RST}"; read -r svc_yes

if [[ "$svc_yes" =~ ^[Yy]$ ]]; then
    step 7 7 "Enabling monitoring services"

    # Pause monitor daemon
    cat > /usr/local/bin/opudp_pausemon.sh <<'DEOF'
#!/bin/bash
...
DEOF
    chmod +x /usr/local/bin/opudp_pausemon.sh

    cat > /etc/systemd/system/opudp-pausemon.service <<'POFF'
[Unit]
...
Type=simple
User=root
ExecStart=/usr/local/bin/opudp_pausemon.sh
Restart=always
RestartSec=10
[Install]
WantedBy=multi-user.target
POFF
    ...
    systemctl start  opudp-pausemon 2>/dev/null
    ok "Pause monitor enabled"

    echo -ne "  ${GRN}Enable IP limit monitor? (checks every 5s) [y/N]: ${RST}"; read -r ipm_yes
    [[ "$ipm_yes" =~ ^[Yy]$ ]] && /usr/local/bin/onepesewa --bot-ipmon_on 2>/dev/null && ok "IP limit monitor enabled"

    echo -ne "  ${GRN}Enable bandwidth monitor? (updates every 30s) [y/N]: ${RST}"; read -r bwm_yes
    [[ "$bwm_yes" =~ ^[Yy]$ ]] && /usr/local/bin/onepesewa --bot-bwmon_on 2>/dev/null && ok "Bandwidth monitor enabled"

    echo -ne "  ${GRN}Enable connection limit enforcement? (cron every min) [y/N]: ${RST}"; read -r enf_yes
    [[ "$enf_yes" =~ ^[Yy]$ ]] && /usr/local/bin/onepesewa --bot-enforce_on 2>/dev/null && ok "Connection enforcement enabled"
fi
...
chmod +x /tmp/opudp_install.sh
sudo /tmp/opudp_install.sh
